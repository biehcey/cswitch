 ...
 * chain reversed: config record out, then Bindings pointing at it, then — last, because
 * it's the irreversible step — the physical profile directory. Record removal and
 * Binding cleanup are computed in memory and land as one atomic config.json write; if
 * that write fails, the directory is never touched. If the directory delete itself
 * fails (already committed config.json is the source of truth by that point), the
 * removal is still reported as done, with a warning attached.
 */
export function removeProfile(params: RemoveProfileParams): RemoveProfileResult {
  ...
  try {
    ...
    return {
      kind: "ok",
      warning: `cswitch: removed "${name}" from config, but could not delete its directory: ${(err as Error).message}`,
    };
  }
